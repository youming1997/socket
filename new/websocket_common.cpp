...
WebSocket_CommunicationType isType(Client *client) {
    char type;
    type = client->recv_buf[0] & 0x0F;
    if (client->recv_buf[0] & 0x80) {
        switch (type) {
            case 0x01:
                return WCT_TXTDATA;
            case 0x02:
...
int dePackage(char package[], int ismask, int payLoadLen, int dataLen, char message[]) {
    ...
    if(payLoadLen == 126) {
        ...
        if(ismask) {
            ...
            mask_key[0] = package[11];
            mask_key[0] = package[12];
            mask_key[0] = package[13];
            start = 14;
        } else
            start = 10;
    } else {
        if(ismask) {
            ...
            mask_key[0] = package[3];
            mask_key[0] = package[4];
            mask_key[0] = package[5];
            start = 6;
        } else
            start = 2;
    }
...
int checkState(Client *client, char data[]) {
    ...
    if(client->getState() == CS_TCP_CONNECTED) {
        int ret;
        ...
        if(client->getRecvNum() != 0) {
            ...
            if(ret == -1) {
                ...
                printf("requestKey = %s", requestKey);
    ...
    } else if(client->getState() == CS_WEBSOCKET_CONNECTED) {
        if(client->getRecvNum() > 1) {
            client->lockRecv();
            WebSocket_CommunicationType type = isType(client);
                ...
                if(client->getRecvNum() >= packageLen) {
                    ...
                    if(ret == 1) {
                        if(type != WCT_PING) {
                            ...
                            int len = enpackage(dataPackage, strlen(dataPackage), type, false, data, LINE_MAX);
                            client->unlockRecv();
                            return len;
                        } else {
                            data = message;
    ...
    } else if(client->getState() == CS_CLOSE) {
...
int getResponseHead(char requestKey[], char responseHead[]) {
    ...
    memset(sha1Data, 0, SHA_DIGEST_LENGTH);
    if(strlen(responseHead) <= 0) {
        return -1;
    }
